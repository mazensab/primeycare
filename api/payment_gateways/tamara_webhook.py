...
# 🧠 Primey Care | Tamara Webhook API V2
# ------------------------------------------------------------
# ✅ استقبال Webhook من Tamara
# ✅ قراءة JSON آمنة
# ✅ استخراج tamaraToken من body أو headers
# ✅ يستدعي payment_gateways.services.handle_tamara_webhook
# ✅ عند نجاح الدفع:
#    Tamara Webhook
...
def _extract_tamara_token(request, payload: dict[str, Any]) -> str:
    """
    Tamara token قد يصل داخل body أو headers بأكثر من اسم.
    """
    candidates = [
        payload.get("tamaraToken"),
        payload.get("tamara_token"),
        payload.get("notification_token"),
        payload.get("token"),
        request.headers.get("tamaraToken"),
        request.headers.get("TamaraToken"),
        request.headers.get("Tamara-Token"),
        request.headers.get("X-Tamara-Token"),
        request.headers.get("X-TamaraToken"),
        request.headers.get("Authorization"),
        request.META.get("HTTP_TAMARATOKEN"),
        request.META.get("HTTP_TAMARA_TOKEN"),
        request.META.get("HTTP_X_TAMARA_TOKEN"),
        request.META.get("HTTP_X_TAMARATOKEN"),
...
def tamara_webhook_api(request):
    ...
    - كل المعالجة تمر من:
      payment_gateways.services.handle_tamara_webhook
    ...
    try:
        ...
        incoming_token = _extract_tamara_token(request, payload)
        ...
        if not is_success:
            return _json_error(
                result.get("message") or "Tamara webhook rejected.",
                status=403,
                data=result,
            )
        ...
        return _json_success(
            result,
            message=result.get("message") or "Tamara webhook processed successfully.",
            status=200,
        )

    except ValidationError as exc:
        return _json_error(
            "بيانات Tamara Webhook غير صحيحة.",
            status=400,
            errors=_validation_errors(exc),
        )

    except PaymentGatewayNotConfiguredError as exc:
        logger.warning("Tamara gateway is not configured while processing webhook: %s", exc)
        return _json_error(str(exc), status=503)

    except PaymentGatewayValidationError as exc:
        logger.warning("Tamara webhook validation error: %s", exc)
        return _json_error(str(exc), status=400)

    except PaymentGatewayServiceError as exc:
        logger.warning("Tamara webhook service error: %s", exc)
        return _json_error(str(exc), status=502)

    except Exception as exc:
        logger.exception("Unexpected error while processing Tamara webhook: %s", exc)
        return _json_error(
            "Unexpected error while processing Tamara webhook.",
            status=500,
        )