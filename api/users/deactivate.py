...
# 🚀 الإصدار: Users Deactivate API V1.2
# ---------------------------------------------------------------
...
def _deactivate_user_sessions(user: Any) -> int:
    now = timezone.now()

    active_sessions = ActiveUserSession.objects.filter(
        user=user,
        is_active=True,
    )
...
def users_deactivate_api(request, user_id: int):
    if request.user.id == user_id:
        return _json_error(
            ...
            errors={
                "user_id": [
                    "لا يمكنك تعطيل حسابك الحالي من نفس الجلسة."
                ]
            },
        )
    ...
    with transaction.atomic():
        ...
        sessions_deactivated_count = _deactivate_user_sessions(user)
    ...
    return _json_success(
        {
            ...
            "data": {
                ...
                "deactivation": {
                    "changed": True,
                    "sessions_deactivated_count": sessions_deactivated_count,
                },
            },
            "deactivation": {
                "changed": True,
