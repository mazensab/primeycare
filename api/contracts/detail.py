...
# 🧠 Primey Care | Contracts API Detail/Update/Terminate V2
# ------------------------------------------------------------
# ✅ Contract Detail
# ✅ Contract Update
# ✅ Safe Delete as TERMINATED
# ✅ ContractProduct offers loaded with product/provider
# ✅ Authenticated Access
# ✅ عرض حساب دخول مقدم الخدمة المرتبط Provider.user
# ✅ إنشاء حساب دخول مقدم الخدمة اختياريًا من تفاصيل العقد
# ------------------------------------------------------------
# القاعدة المعتمدة:
# - Product = كتالوج ثابت
...
# - ContractProduct = عرض/سعر/خصم المنتج حسب العقد ومقدم الخدمة
# ============================================================
...
def _safe_set_updated_by(instance: Contract, user) -> None:
    ...
    *,
    ...
    return create_actor_user(
        ...
        extra_data={
            ...
            "entity_type": "provider",
            "entity_id": provider.pk,
            "source": "contracts_detail_provider_login_user",
    ...
    if request.method == "PATCH":
        ...
        try:
            ...
            response_payload = _contract_response_payload(
                contract=contract,
                message=(
                    "Contract updated and provider login user created successfully."
                    if create_provider_login_user and provider_login_result
                    else "Contract updated successfully."
                ),
                provider_login_result=provider_login_result,
            )
            ...
            return _json_error(
                "Unexpected error while updating contract.",
                500,
            )
