...
function roleLabel(value: string | null | undefined, locale: AppLocale) {
  ...
  const item = map[role.toLowerCase()];
  if (!item) return role;
  ...
  const stats = useMemo(() => {
    ...
    const system = users.filter(
      (item) =>
        item.is_staff ||
        item.is_superuser ||
        item.profile.user_type === "SYSTEM" ||
        item.profile.user_type === "SUPER_ADMIN" ||
        item.profile.role === "system_admin",
    ).length;
  ...
  async function loadUsers() {
    ...
    try {
      ...
      const response = await fetch(
        `${getApiBaseUrl()}/api/users/?${params.toString()}`,
        {
          method: "GET",
          credentials: "include",
          headers: {
          ...
          <Card className="rounded-3xl border-0 shadow-sm">
            <CardHeader>
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-base">{labels.filters}</CardTitle>
              </div>
            </CardHeader>
                  ...
                  <table className="w-full min-w-[980px] text-sm">
                    <thead className="bg-slate-50 text-slate-500">
                      <tr>
                        ...
                        <th className="px-5 py-4 text-end font-semibold">
                          {labels.actions}
                        </th>
                      </tr>
                    </thead>
