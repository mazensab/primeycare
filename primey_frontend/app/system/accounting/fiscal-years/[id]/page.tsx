...
/* ============================================================
   ...
   🧾 Primey Care — Fiscal Year Details
   ------------------------------------------------------------
   ✅ Approved Products / Customers / Orders detail pattern
   ✅ Real API:
      GET /api/accounting/fiscal-years/{id}/
      fallback:
      GET /api/accounting/fiscal_years/{id}/
      GET /api/accounting/years/{id}/
      GET /api/accounting/reports/fiscal-years/{id}/
      GET /api/accounting/periods/?page=1&page_size=500
   ✅ Profile side card + KPI cards + related periods table
   ✅ Search / status / sort / columns / pagination
   ✅ Excel .xls + Web print
   ✅ Skeleton loading
...
type SortKey = "newest" | "oldest" | "period" | "status" | "journals_high" | "debit_high" | "credit_high";
...
const translations = {
  ar: {
    title: "تفاصيل السنة المالية",
    subtitle: "عرض بيانات السنة المالية والفترات المحاسبية المرتبطة بها.",
    ...
    profileTitle: "بطاقة السنة المالية",
    profileDesc: "ملخص السنة وحالة الإقفال.",
    infoTitle: "بيانات السنة المالية",
    infoDesc: "النطاق الزمني والحالة والملاحظات.",
    periodsTitle: "الفترات المحاسبية",
    periodsDesc: "الفترات المرتبطة بهذه السنة المالية.",
    ...
    all: "الكل",
    searchPlaceholder: "ابحث باسم الفترة أو الكود أو الحالة...",
    statusFilter: "حالة الفترة",
    sort: "الترتيب",
    columns: "الأعمدة",
    ...
    previous: "السابق",
    next: "التالي",
    noDataTitle: "لا توجد فترات مرتبطة",
    noDataDesc: "ستظهر الفترات المحاسبية المرتبطة بهذه السنة هنا.",
    noResultsTitle: "لا توجد نتائج مطابقة",
    noResultsDesc: "غيّر البحث أو الفلاتر لعرض نتائج أخرى.",
    errorTitle: "تعذر تحميل تفاصيل السنة المالية",
    errorDesc: "تأكد من تشغيل الباكند ثم أعد المحاولة.",
    notFoundTitle: "السنة المالية غير موجودة",
    notFoundDesc: "لم يتم العثور على السنة المالية المطلوبة أو ربما تم حذفها.",
    tryAgain: "إعادة المحاولة",
    refreshed: "تم تحديث تفاصيل السنة المالية.",
    exportEmpty: "لا توجد بيانات للتصدير.",
    printEmpty: "لا توجد بيانات للطباعة.",
    printTitle: "تقرير تفاصيل السنة المالية",
  ...
  en: {
    ...
    subtitle: "View fiscal year details and related accounting periods.",
    back: "Fiscal years",
    refresh: "Refresh",
    export: "Export Excel",
    print: "Print",
    ...
    profileTitle: "Fiscal year card",
    profileDesc: "Year summary and closing status.",
    infoTitle: "Fiscal year information",
    infoDesc: "Date range, status, and notes.",
    periodsTitle: "Accounting periods",
    periodsDesc: "Periods linked to this fiscal year.",
    ...
    all: "All",
    searchPlaceholder: "Search by period name, code, or status...",
    statusFilter: "Period status",
    sort: "Sort",
    columns: "Columns",
    ...
    previous: "Previous",
    next: "Next",
    noDataTitle: "No linked periods",
    noDataDesc: "Accounting periods linked to this fiscal year will appear here.",
    noResultsTitle: "No matching results",
    noResultsDesc: "Change search or filters to show other results.",
    errorTitle: "Unable to load fiscal year details",
    errorDesc: "Make sure the backend is running, then try again.",
    notFoundTitle: "Fiscal year not found",
    notFoundDesc: "The requested fiscal year was not found or may have been deleted.",
    tryAgain: "Try again",
    refreshed: "Fiscal year details refreshed.",
    exportEmpty: "No data to export.",
    printEmpty: "No data to print.",
    printTitle: "Fiscal year details report",
...
function normalizeFiscalYearStatus(value: unknown, isClosed: boolean, isLocked: boolean): FiscalYearStatus {
...
function normalizePeriodStatus(value: unknown, isClosed: boolean, isLocked: boolean): PeriodStatus {
  const status = normalizeText(value).toLowerCase();
...
function normalizeFiscalYear(value: unknown, fallbackId = ""): FiscalYearRecord {
  ...
  const status = normalizeFiscalYearStatus(
    item.status || item.year_status || item.fiscal_year_status,
    isClosed,
    isLocked,
  );
...
function normalizePeriod(value: unknown): PeriodRecord {
  const item = asRecord(value);
  const fiscalYear = asRecord(item.fiscal_year || item.fiscalYear || item.year || item.fiscal_year_object);

  const id = normalizeText(item.id || item.pk || item.uuid);
  const isClosed = toBoolean(item.is_closed ?? item.closed ?? item.isClosed, false);
  const isLocked = toBoolean(item.is_locked ?? item.locked ?? item.isLocked, false);
  const status = normalizePeriodStatus(item.status || item.period_status, isClosed, isLocked);

  const code = normalizeText(item.code || item.period_code || item.number);
  const name =
  ...
  return {
    ...
    code,
    name,
    fiscal_year_id: normalizeText(item.fiscal_year_id || fiscalYear.id || fiscalYear.pk),
...
function fiscalYearStatusLabel(status: FiscalYearStatus, locale: Locale) {
...
function periodStatusLabel(status: PeriodStatus, locale: Locale) {
  const t = translations[locale];
...
function getFiscalYearStatusClass(status: FiscalYearStatus) {
  if (status === "open") return "border-emerald-500/30 bg-emerald-50 text-emerald-700 hover:bg-emerald-50";
  if (status === "locked") return "border-blue-500/30 bg-blue-50 text-blue-700 hover:bg-blue-50";
  if (status === "closed") return "border-slate-500/30 bg-slate-50 text-slate-700 hover:bg-slate-50";
...
function FiscalYearStatusBadge({ status, locale }: { status: FiscalYearStatus; locale: Locale }) {
  return (
    <Badge
      variant="outline"
      className={cn("rounded-full px-2.5 py-1 text-xs font-medium", getFiscalYearStatusClass(status))}
    >
      {fiscalYearStatusLabel(status, locale)}
    </Badge>
  );
}
...
function PeriodStatusBadge({ status, locale }: { status: PeriodStatus; locale: Locale }) {
  return (
    ...
    >
      {periodStatusLabel(status, locale)}
    </Badge>
  );
}
...
export default function AccountingFiscalYearDetailsPage() {
  const params = useParams<{ id?: string }>();
  const fiscalYearId = decodeURIComponent(String(params?.id || ""));

  const [locale, setLocale] = React.useState<Locale>("ar");
  const [fiscalYear, setFiscalYear] = React.useState<FiscalYearRecord | null>(null);
  ...
  const loadDetails = React.useCallback(
    async ({ silent = false }: { silent?: boolean } = {}) => {
      ...
      try {
        ...
        const detailEndpoints = [
          ...
          `/api/accounting/fiscal_years/${fiscalYearId}/`,
          `/api/accounting/years/${fiscalYearId}/`,
        ...
        const yearRecord = normalizeFiscalYear(extractObject(detailPayload), fiscalYearId);
        setFiscalYear(yearRecord);
        ...
        const periodPayload = await fetchJson<ApiResponse>(
          makeApiUrl("/api/accounting/periods/", periodParams),
          controller.signal,
        ).catch(() => null);

        const periodRows = periodPayload ? extractArray(periodPayload).map(normalizePeriod) : [];

        const linkedPeriods = periodRows.filter((period) => {
          return (
            period.fiscal_year_id === yearRecord.id ||
            period.fiscal_year_id === fiscalYearId ||
            period.fiscal_year_name === yearRecord.name ||
            period.fiscal_year_name === yearRecord.code
          );
        });

        setPeriods(linkedPeriods);
    ...
    [fiscalYearId, t.errorDesc, t.refreshed],
  );
  ...
  const summary = React.useMemo(() => {
    const periodsCount = periods.length || fiscalYear?.periods_count || 0;
    const journals = periods.length
      ? periods.reduce((sum, period) => sum + period.journals_count, 0)
      : fiscalYear?.journals_count || 0;
    const debit = periods.length
      ? periods.reduce((sum, period) => sum + period.total_debit, 0)
      : fiscalYear?.total_debit || 0;
    const credit = periods.length
      ? periods.reduce((sum, period) => sum + period.total_credit, 0)
      : fiscalYear?.total_credit || 0;
  ...
  const filteredPeriods = React.useMemo(() => {
    ...
    return result;
  }, [periods, searchInput, sortKey, statusFilter]);
  ...
  function buildExportRows() {
    return filteredPeriods.map((period) => ({
      period: period.name || t.notAvailable,
      code: period.code || t.notAvailable,
      startDate: formatDate(period.start_date),
      endDate: formatDate(period.end_date),
      status: periodStatusLabel(period.status, locale),
  ...
  function exportExcel() {
    ...
    const html = `
      <html>
        ...
        <body>
          ...
          <p>${escapeHtml(t.fiscalYear)}: ${escapeHtml(fiscalYear?.name || "")}</p>
          <p>${escapeHtml(t.code)}: ${escapeHtml(fiscalYear?.code || fiscalYear?.id || "")}</p>
    ...
    link.href = url;
    link.download = `primey-care-fiscal-year-${fiscalYear?.code || fiscalYearId}-${new Date()
      .toISOString()
      .slice(0, 10)}.xls`;
    link.click();
    ...
    printWindow.document.write(`
      ...
      <html lang="${locale}" dir="${dir}">
        ...
        <body>
          ...
          <div class="summary">
            <div class="box"><span>${escapeHtml(t.totalPeriods)}</span><strong>${escapeHtml(summary.periodsCount)}</strong></div>
            <div class="box"><span>${escapeHtml(t.totalJournals)}</span><strong>${escapeHtml(summary.journals)}</strong></div>
        ...
        <div className="space-y-4">
          <Card className="rounded-lg border bg-card shadow-none">
            ...
            <CardContent className="space-y-3 px-6 pb-6">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="truncate text-base font-semibold text-foreground">
                      {fiscalYear?.name || t.notAvailable}
                    </p>
                    <p className="mt-1 truncate text-sm text-muted-foreground tabular-nums">
                      {fiscalYear?.code || fiscalYear?.id || t.notAvailable}
                    </p>
                  </div>

                  <FiscalYearStatusBadge status={fiscalYear?.status || "unknown"} locale={locale} />
                </div>
              </div>
              ...
              <DetailLine
                ...
                value={
                  <button
                    ...
                    onClick={() => void copyValue(fiscalYear?.code || fiscalYear?.id || "")}
                  >
              ...
              <DetailLine
                label={t.startDate}
                value={<span className="tabular-nums">{formatDate(fiscalYear?.start_date)}</span>}
              />

              <DetailLine
                label={t.endDate}
                value={<span className="tabular-nums">{formatDate(fiscalYear?.end_date)}</span>}
              />

              <DetailLine
                label={t.status}
                value={<FiscalYearStatusBadge status={fiscalYear?.status || "unknown"} locale={locale} />}
              />
          ...
          <Card className="rounded-lg border bg-card shadow-none">
            ...
            <CardContent className="space-y-3 px-6 pb-6">
              <DetailLine
                label={t.createdAt}
                value={<span className="tabular-nums">{formatDateTime(fiscalYear?.created_at)}</span>}
              />

              <DetailLine
                label={t.updatedAt}
                value={<span className="tabular-nums">{formatDateTime(fiscalYear?.updated_at)}</span>}
              />

              <div className="rounded-lg border bg-background p-4">
                <p className="mb-2 text-sm font-medium text-foreground">{t.notes}</p>
                <p className="text-sm leading-6 text-muted-foreground">
                  {fiscalYear?.notes || t.notAvailable}
                </p>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="space-y-4">
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            <KpiCard
              ...
              value={formatInteger(summary.periodsCount)}
              trend={t.periodsTitle}
              icon={CalendarClock}
            />
            ...
            <KpiCard
              ...
              trend={summary.difference < 0.01 ? t.yes : t.no}
              icon={summary.difference < 0.01 ? ShieldCheck : XCircle}
            />
          </div>
          ...
          <Card className="overflow-hidden rounded-lg border bg-card shadow-none">
            <CardHeader className="px-4 py-4">
              ...
              <CardDescription>{t.periodsDesc}</CardDescription>
            </CardHeader>
                      ...
                      {pageRows.length ? (
                        pageRows.map((period) => (
                          <TableRow key={period.id || period.code || period.name} className="h-[62px]">
                            ...
                            {columns.status ? (
                              <TableCell className="h-[62px] w-[120px] overflow-hidden px-4 text-right align-middle">
                                <PeriodStatusBadge status={period.status} locale={locale} />
                              </TableCell>
                            ) : null}
                              ...
                              <TableCell className="h-[62px] w-[85px] overflow-hidden px-4 text-center align-middle">
                                {period.id ? (
                                  <Button asChild variant="ghost" size="sm" className="h-8 rounded-lg">
                                    <Link
                                      href={`/system/accounting/periods/${encodeURIComponent(period.id)}`}
                                      title={t.openDetails}
                                    >
