...
/* ============================================================
   ...
   ------------------------------------------------------------
   ✅ نفس تنسيق صفحات المحاسبة المرفقة
...
type SectionFilter = "ALL" | BalanceSection;
...
function buildBalanceSheetPath({
  asOfDate,
  includeZeroAccounts,
  postedOnly,
  ...
  const basePath = excel
    ? "/api/accounting/reports/balance-sheet/excel/"
  ...
  return `${basePath}${buildQuery({
    ...
    include_current_year_earnings: includeCurrentYearEarnings,
  })}`;
}

async function fetchBalanceSheet(path: string): Promise<BalanceSheetPayload> {
  ...
  const payload = (await response.json()) as ApiEnvelope<BalanceSheetPayload>;

  if (payload.ok === false) {
    throw new Error(payload.message || "Balance sheet request failed");
  }

  if (!payload.data) {
    throw new Error("Balance sheet response does not contain data");
  }
...
function dictionary(locale: AppLocale) {
  ...
  return {
    ...
    subtitle: isArabic
      ? "عرض الأصول والالتزامات وحقوق الملكية مع حالة التوازن حتى تاريخ محدد."
      : "View assets, liabilities, and equity with balance status as of a selected date.",
    ...
    statusTitle: isArabic ? "حالة المركز المالي" : "Balance Sheet Status",
    statusDesc: isArabic
      ? "تحليل سريع للأصول والالتزامات وحقوق الملكية وحالة التوازن."
      : "Quick analysis of assets, liabilities, equity, and balance status.",

    summaryTitle: isArabic ? "ملخص المركز المالي" : "Balance Sheet Summary",
    summaryDesc: isArabic
      ? "أهم المؤشرات المالية الحالية للمركز المالي."
      : "Key financial indicators for the current balance sheet.",

    assets: isArabic ? "الأصول" : "Assets",
    liabilities: isArabic ? "الالتزامات" : "Liabilities",
    equity: isArabic ? "حقوق الملكية" : "Equity",
    liabilitiesAndEquity: isArabic
      ? "الالتزامات وحقوق الملكية"
      : "Liabilities & Equity",
    difference: isArabic ? "الفرق" : "Difference",

    totalAssets: isArabic ? "إجمالي الأصول" : "Total Assets",
    totalLiabilities: isArabic ? "إجمالي الالتزامات" : "Total Liabilities",
    totalEquity: isArabic ? "إجمالي حقوق الملكية" : "Total Equity",
    ...
    searchPlaceholder: isArabic
      ? "ابحث في حسابات المركز المالي..."
      : "Search balance sheet accounts...",

    columns: isArabic ? "الأعمدة" : "Columns",
    filters: isArabic ? "الفلاتر" : "Filters",
    all: isArabic ? "الكل" : "All",
    assetsOnly: isArabic ? "الأصول" : "Assets",
    liabilitiesOnly: isArabic ? "الالتزامات" : "Liabilities",
    equityOnly: isArabic ? "حقوق الملكية" : "Equity",
    ...
    includeCurrentYearEarnings: isArabic
      ? "إظهار صافي ربح السنة الحالية"
      : "Include current year earnings",
    ...
    noRowsDesc: isArabic
      ? "غيّر الفلاتر أو تأكد من وجود قيود مرحلة على حسابات المركز المالي."
      : "Change filters or make sure posted journals exist for balance sheet accounts.",

    view: isArabic ? "عرض" : "View",
    previous: isArabic ? "السابق" : "Previous",
    next: isArabic ? "التالي" : "Next",

    loadSuccess: isArabic ? "تم تحديث المركز المالي" : "Balance sheet refreshed",
    loadError: isArabic
...
function sectionBadgeClass(section: BalanceSection) {
  if (section === "ASSETS") {
...
export default function BalanceSheetPage() {
  ...
  const [payload, setPayload] = useState<BalanceSheetPayload | null>(null);
  ...
  const [includeCurrentYearEarnings, setIncludeCurrentYearEarnings] =
    useState(true);
  ...
  async function loadBalanceSheet(showToast = false) {
    try {
      setLoading(true);

      const path = buildBalanceSheetPath({
        asOfDate,
        includeZeroAccounts,
        postedOnly,
        includeCurrentYearEarnings,
      ...
      const data = await fetchBalanceSheet(path);
      setPayload(data);
  ...
  const rows = useMemo<BalanceSheetRow[]>(() => {
    const assetRows =
      payload?.assets?.rows?.map((row) => ({
        id: `asset-${row.account_id}-${row.account_code}`,
    ...
    const liabilityRows =
      payload?.liabilities?.rows?.map((row) => ({
        id: `liability-${row.account_id}-${row.account_code}`,
    ...
    const equityRows =
      payload?.equity?.rows?.map((row) => ({
        id: `equity-${row.account_id}-${row.account_code}`,
    ...
    return [...assetRows, ...liabilityRows, ...equityRows];
  }, [payload]);

  const summary = useMemo(() => {
    const assets = payload?.assets?.total_amount || "0.00";
    const liabilities = payload?.liabilities?.total_amount || "0.00";
    const equity = payload?.equity?.total_amount || "0.00";
    const liabilitiesAndEquity =
      payload?.total_liabilities_and_equity || "0.00";

    const assetsValue = toNumber(assets);
    const liabilitiesValue = toNumber(liabilities);
    const equityValue = toNumber(equity);
    const liabilitiesAndEquityValue = toNumber(liabilitiesAndEquity);

    const difference = Math.abs(assetsValue - liabilitiesAndEquityValue);
    const isBalanced = Boolean(payload?.is_balanced) || difference === 0;

    const liabilitiesRatio =
      assetsValue > 0 ? (liabilitiesValue / assetsValue) * 100 : 0;

    const equityRatio = assetsValue > 0 ? (equityValue / assetsValue) * 100 : 0;
  ...
  const statusCards = [
    {
      ...
      icon: TrendingUp,
      percent: summary.assetsValue > 0 ? 100 : 0,
      money: true,
    },
    {
      ...
      icon: TrendingDown,
      percent: Math.min(summary.liabilitiesRatio, 100),
      money: true,
    },
    {
      label: t.totalEquity,
      ...
      icon: WalletCards,
      percent: Math.min(summary.equityRatio, 100),
      money: true,
    },
    {
      label: t.difference,
                      ...
                      <div className="h-2 rounded-full bg-slate-100">
                        <div
                          className={`h-2 rounded-full ${
                            card.label === t.totalLiabilities
                              ? "bg-sky-500"
                              : card.label === t.totalEquity
                                ? "bg-violet-500"
                  ...
                  >
                    <div dir={isArabic ? "rtl" : "ltr"}>
                      ...
                      <div className="space-y-3 p-2">
                        ...
                        <label className="flex items-center gap-2 rounded-xl border border-slate-200 p-2 text-sm">
                          <Checkbox
                            checked={includeCurrentYearEarnings}
                            onCheckedChange={(value) =>
                              setIncludeCurrentYearEarnings(Boolean(value))
                            }
                          />
                          <span>{t.includeCurrentYearEarnings}</span>
                        </label>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="rounded-2xl border border-dashed border-slate-200 p-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-xl bg-slate-50 p-3">
                    <p className="text-xs text-slate-500">{t.assets}</p>
                    <p className="mt-1 text-lg font-bold text-slate-950">
                      <MoneyValue value={summary.assets} />
                    </p>
                  </div>

                  <div className="rounded-xl bg-slate-50 p-3">
                    <p className="text-xs text-slate-500">{t.liabilities}</p>
                    <p className="mt-1 text-lg font-bold text-slate-950">
                      <MoneyValue value={summary.liabilities} />
                    </p>
                  </div>

                  <div className="rounded-xl bg-slate-50 p-3">
                    <p className="text-xs text-slate-500">{t.equity}</p>
                    <p className="mt-1 text-lg font-bold text-slate-950">
                      <MoneyValue value={summary.equity} />
                    </p>
                  </div>
              ...
              <div className="grid gap-2">
                ...
                <Button
                  ...
                  variant="outline"
                  className="h-10 justify-between rounded-xl bg-white"
                  onClick={() => setSectionFilter("ASSETS")}
                >
                  <span>{t.assets}</span>
                ...
                <Button
                  ...
                  variant="outline"
                  className="h-10 justify-between rounded-xl bg-white"
                  onClick={() => setSectionFilter("LIABILITIES")}
                >
