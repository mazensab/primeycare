...
/* ============================================================
   📂 app/system/treasury/vouchers/payment/[id]/page.tsx
   🧠 Primey Care | Payment Voucher Details Page
   ------------------------------------------------------------
...
const translations = {
  ar: {
    ...
    subtitle: "عرض بيانات سند الصرف وحالته وأثره على الخزينة.",
    back: "سندات الخزينة",
    treasury: "الخزينة",
    ...
    summary: "ملخص سند الصرف",
    summaryDesc: "بيانات السند الأساسية والحالة الحالية.",
    financialSummary: "الملخص المالي",
    financialSummaryDesc: "المبلغ والرسوم والصافي الخارج من الخزينة.",
    accountInfo: "حساب الخزينة",
    accountInfoDesc: "الحساب الذي سيتم الصرف منه.",
    referenceInfo: "الطرف والمرجع",
    referenceInfoDesc: "بيانات الطرف والمرجع والوصف.",
    postingInfo: "حالة الأثر والترحيل",
    ...
    confirmQuestion: "هل تريد تأكيد سند الصرف؟",
    cancelQuestion: "هل تريد إلغاء سند الصرف؟",
    confirmedToast: "تم تأكيد سند الصرف بنجاح.",
    cancelledToast: "تم إلغاء سند الصرف بنجاح.",
    refreshed: "تم تحديث تفاصيل سند الصرف.",

    errorTitle: "تعذر تحميل تفاصيل سند الصرف",
    errorDesc: "تأكد من تشغيل الباكند ثم أعد المحاولة.",
    notFoundTitle: "لم يتم العثور على سند الصرف",
    notFoundDesc: "قد يكون السند غير موجود أو لا تملك صلاحية عرضه.",
    tryAgain: "إعادة المحاولة",
    actionError: "تعذر تنفيذ الإجراء.",
    printTitle: "تفاصيل سند الصرف",
  ...
  en: {
    ...
    subtitle: "View payment voucher data, status, and treasury impact.",
    ...
    summary: "Payment voucher summary",
    summaryDesc: "Main voucher data and current status.",
    financialSummary: "Financial summary",
    financialSummaryDesc: "Amount, fees, and outgoing net amount.",
    accountInfo: "Treasury account",
    accountInfoDesc: "The account this payment is paid from.",
    referenceInfo: "Party and reference",
    referenceInfoDesc: "Party, reference, and description data.",
    postingInfo: "Impact and posting status",
    ...
    confirmQuestion: "Confirm this payment voucher?",
    cancelQuestion: "Cancel this payment voucher?",
    confirmedToast: "Payment voucher confirmed successfully.",
    cancelledToast: "Payment voucher cancelled successfully.",
    refreshed: "Payment voucher details refreshed.",

    errorTitle: "Unable to load payment voucher details",
    errorDesc: "Make sure the backend is running, then try again.",
    notFoundTitle: "Payment voucher not found",
    notFoundDesc: "The voucher may not exist or you may not have permission to view it.",
    tryAgain: "Try again",
    actionError: "Unable to complete the action.",
    printTitle: "Payment voucher details",
...
function toBoolean(value: unknown, fallback = false) {
  ...
  if (typeof value === "string") {
    ...
    if (["1", "true", "yes", "on", "applied", "posted", "confirmed"].includes(normalized)) {
      return true;
    }
...
function normalizeVoucher(value: unknown): PaymentVoucher {
  ...
  return {
    ...
    transaction_type: normalizeText(item.transaction_type || item.type || "expense").toLowerCase(),
...
export default function TreasuryPaymentVoucherDetailsPage() {
  ...
  const [voucher, setVoucher] = React.useState<PaymentVoucher | null>(null);
  ...
  const loadVoucher = React.useCallback(
    async ({ silent = false }: { silent?: boolean } = {}) => {
      ...
      try {
        ...
        const normalized = normalizeVoucher(extractPayload(payload));
  ...
  async function runAction(action: "confirm" | "cancel") {
    ...
    try {
      const endpoint =
        action === "confirm"
          ? `/api/treasury/transactions/${encodeURIComponent(voucher.id)}/confirm/`
          : `/api/treasury/transactions/${encodeURIComponent(voucher.id)}/cancel/`;
      ...
      await loadVoucher({ silent: false });
    } catch (caughtError) {
      const message =
        caughtError instanceof Error && caughtError.message ? caughtError.message : t.actionError;
    ...
    printWindow.document.write(`
      ...
      <html lang="${locale}" dir="${dir}">
        ...
        <body>
          ...
          <div class="grid">
            <div class="box"><span>${escapeHtml(t.number)}</span><strong>${escapeHtml(voucher.transaction_number || t.notAvailable)}</strong></div>
            <div class="box"><span>${escapeHtml(t.status)}</span><strong>${escapeHtml(statusLabel(voucher.status, locale))}</strong></div>
            <div class="box"><span>${escapeHtml(t.date)}</span><strong>${escapeHtml(formatDate(voucher.transaction_date))}</strong></div>
            <div class="box"><span>${escapeHtml(t.account)}</span><strong>${escapeHtml(voucher.treasury_account_name || t.notAvailable)}</strong></div>
            <div class="box"><span>${escapeHtml(t.amount)}</span><strong>${escapeHtml(formatMoney(voucher.amount))}</strong></div>
            <div class="box"><span>${escapeHtml(t.fees)}</span><strong>${escapeHtml(formatMoney(voucher.fees_amount))}</strong></div>
            <div class="box"><span>${escapeHtml(t.net)}</span><strong>${escapeHtml(formatMoney(voucher.net_amount))}</strong></div>
            <div class="box"><span>${escapeHtml(t.party)}</span><strong>${escapeHtml(voucher.party_name || t.notAvailable)}</strong></div>
            <div class="box"><span>${escapeHtml(t.reference)}</span><strong>${escapeHtml(voucher.reference || t.notAvailable)}</strong></div>
            <div class="box"><span>${escapeHtml(t.description)}</span><strong>${escapeHtml(voucher.description || t.notAvailable)}</strong></div>
          </div>
        ...
        <div className="space-y-4">
          <Card className="rounded-lg border bg-card shadow-none">
            <CardHeader className="relative px-6 py-5">
              ...
              <CardTitle>{voucher.transaction_number || t.notAvailable}</CardTitle>
              <CardAction>
            ...
            <CardContent className="space-y-3 px-6 pb-6">
              <div className="rounded-lg border bg-background p-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold text-foreground">{t.paymentVoucher}</span>
                </div>
              </div>
              ...
              <div className="rounded-lg border bg-background p-4">
                <div className="mb-1 text-xs text-muted-foreground">{t.date}</div>
          ...
          <Card className="rounded-lg border bg-card shadow-none">
            <CardContent className="flex items-start gap-3 p-4 text-right">
              ...
              <div className="space-y-1">
                <p className="text-sm font-semibold text-foreground">{t.postingInfo}</p>
                <p className="text-sm leading-6 text-muted-foreground">{t.postingInfoDesc}</p>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="space-y-4">
          <Card className="rounded-lg border bg-card shadow-none">
            ...
            <CardContent className="grid gap-3 px-6 pb-6 md:grid-cols-2">
              <DetailItem label={t.account} value={voucher.treasury_account_name || t.notAvailable} />
              <DetailItem label={t.accountCode} value={voucher.treasury_account_code || t.notAvailable} mono />
              <DetailItem label={t.source} value={voucher.source_label || voucher.source || t.notAvailable} />
              <DetailItem label={t.status} value={<StatusBadge status={voucher.status} locale={locale} />} />
            </CardContent>
          </Card>
          ...
          <Card className="rounded-lg border bg-card shadow-none">
            ...
            <CardContent className="grid gap-3 px-6 pb-6 md:grid-cols-2">
              <DetailItem label={t.party} value={voucher.party_name || t.notAvailable} />
              <DetailItem label={t.reference} value={voucher.reference || t.notAvailable} mono />
              <DetailItem label={t.externalReference} value={voucher.external_reference || t.notAvailable} mono />
              <DetailItem label={t.sourceNumber} value={voucher.source_number || t.notAvailable} mono />
              <div className="md:col-span-2">
                <DetailItem label={t.description} value={voucher.description || t.notAvailable} />
              </div>
              <div className="md:col-span-2">
                <DetailItem label={t.notes} value={voucher.notes || t.notAvailable} />
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="rounded-lg border bg-card shadow-none">
            ...
            <CardContent className="grid gap-3 px-6 pb-6 md:grid-cols-3">
              <DetailItem
                label={t.balanceApplied}
                value={
                  <BooleanBadge
                    value={voucher.balance_applied}
                    trueLabel={t.applied}
              ...
              <DetailItem
                ...
                value={
                  <BooleanBadge
                    value={voucher.accounting_posted}
                    trueLabel={t.posted}
                    falseLabel={t.notPosted}
                  />
                }
          ...
          <Card className="rounded-lg border bg-card shadow-none">
            <CardHeader className="px-6 py-5">
              ...
              <CardDescription>{voucher.created_by_name || t.notAvailable}</CardDescription>
            </CardHeader>

            <CardContent className="grid gap-3 px-6 pb-6 md:grid-cols-2">
              <DetailItem label={t.createdAt} value={formatDate(voucher.created_at)} mono />
              <DetailItem label={t.updatedAt} value={formatDate(voucher.updated_at)} mono />
              <DetailItem label={t.confirmedAt} value={formatDate(voucher.confirmed_at)} mono />
              <DetailItem label={t.cancelledAt} value={formatDate(voucher.cancelled_at)} mono />

              {voucher.cancellation_reason ? (
                <div className="md:col-span-2">
                  <DetailItem label={t.cancellationReason} value={voucher.cancellation_reason} />
                </div>
              ) : null}
            </CardContent>
          </Card>
